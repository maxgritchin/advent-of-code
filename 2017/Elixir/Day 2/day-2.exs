...
defmodule Checksum do
  def calc_rows([row | rest], row_values) when row == "", do: calc_rows(rest, row_values)
  def calc_rows([], row_values), do: row_values
  def calc_rows([row | rest], row_values) do
    calc_rows(rest, [get_row_value(row) | row_values])
  end

  def get_row_value(row) when is_binary(row) do
    row
    |> String.replace("\t", " ")
    |> String.split(" ")
  ...
  def get_diff_of_values(row) do
    get_max(row) - get_min(row)
  end
  defp get_min(row) do
    row
    |> Enum.sort()
